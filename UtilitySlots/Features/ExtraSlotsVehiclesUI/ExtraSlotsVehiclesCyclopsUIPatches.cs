...
{
    /// <summary>
    /// UI PDA pour les modules du Cyclops (UpgradeConsole).
    /// - N'agit que si l'Equipment appartient à une UpgradeConsole de Cyclops.
    /// - Clone "Module1" comme template pour les slots UI.
    /// - Crée Module7..N (6 premiers slots vanilla).
    /// - Crée aussi les "screenModule7..N" en clonant "screenModule1"
    ///   pour éviter les logs "slot not found in pda screenModuleX".
    /// </summary>
    [HarmonyPatch(typeof(uGUI_Equipment), nameof(uGUI_Equipment.Init))]
    internal static class ExtraSlotsVehiclesCyclopsUIPatches
    {
        ...
        static void Postfix(uGUI_Equipment __instance, Equipment equipment)
        {
            try
            {
                ...
                // On cible l'UpgradeConsole du Cyclops
                if (ownerGO.GetComponent<UpgradeConsole>() == null)
                    return;
                ...
                if (allSlots == null)
                {
                    Log.Warn("[UtilitySlots][ExtraSlotsVehicles][CyclopsUI] allSlots dictionary is null.");
                    return;
                }
                ...
                if (!allSlots.TryGetValue("Module1", out var module1) || module1 == null)
                {
                    Log.Warn("[UtilitySlots][ExtraSlotsVehicles][CyclopsUI] Could not find Module1 UI slot.");
                    return;
                }
                ...
                {
                    Log.Warn("[UtilitySlots][ExtraSlotsVehicles][CyclopsUI] Module1 RectTransform missing.");
                    return;
                }
                     ...
                     i++)
                {
                    ...
                    int rowIndex = i - ExtraSlotsVehiclesRuntime.VanillaCyclopsModuleSlots;
                ...
                int createdScreens = EnsureCyclopsModuleScreens(__instance, desired);

                if (createdSlots > 0 || createdScreens > 0)
                {
                    Log.Info($"[UtilitySlots][ExtraSlotsVehicles][CyclopsUI] Slots créés/mis à jour={createdSlots}, screens créés={createdScreens}, modules={desired}.");
                }
            }
            catch (Exception e)
            {
                Log.Error("[UtilitySlots][ExtraSlotsVehicles][CyclopsUI] Exception in uGUI_Equipment.Init postfix: " + e);
            }
        }
            ...
            float verticalOffset)
        {
            ...
            if (!allSlots.TryGetValue(slotId, out slot) || slot == null)
            {
                ...
                if (slot == null)
                {
                    ...
                    Log.Info($"[UtilitySlots][ExtraSlotsVehicles][CyclopsUI] Created uGUI slot for '{slotId}'.");
                }
            }
            ...
            Log.Info($"[UtilitySlots][ExtraSlotsVehicles][CyclopsUI] Positioned '{slotId}' at {slotRect.anchoredPosition}.");
        }

        /// <summary>
        /// Crée les GameObjects "screenModule7..N" en clonant "screenModule1"
        /// pour éviter les logs "slot not found in pda screenModuleX".
        /// </summary>
        private static int EnsureCyclopsModuleScreens(uGUI_Equipment ui, int desired)
        {
            int created = 0;
            ...
            var templateScreen = FindDeepChild(root, "screenModule1");
            if (templateScreen == null)
            {
                Log.Warn("[UtilitySlots][ExtraSlotsVehicles][CyclopsUI] screenModule1 not found; PDA screens will stay vanilla.");
                return 0;
            }
            ...
            for (int i = ExtraSlotsVehiclesRuntime.VanillaCyclopsModuleSlots + 1;
                 ...
                 i++)
            {
                string screenName = $"screenModule{i}";
                ...
                created++;
                Log.Info($"[UtilitySlots][ExtraSlotsVehicles][CyclopsUI] Created PDA screen '{screenName}' (clone of screenModule1).");
            }
