...
{
    /// <summary>
    /// UI PDA pour les modules du Cyclops.
    /// - N'agit que si l'Equipment appartient à une UpgradeConsole de Cyclops.
    /// - Construit une grille 4×4 en se basant sur Module1/2/3 (jusqu’à 14).
    /// - Ne déplace pas les slots vanilla (1..6), ajoute seulement 7..14.
    /// </summary>
    ...
    internal static class ExtraSlotsVehiclesCyclopsUIPatches
    {
        ...
        static void Postfix(uGUI_Equipment __instance, Equipment equipment)
        {
            try
            {
                ...
                if (!allSlots.TryGetValue("Module1", out var mod1) || mod1 == null ||
                    !allSlots.TryGetValue("Module2", out var mod2) || mod2 == null ||
                    !allSlots.TryGetValue("Module3", out var mod3) || mod3 == null)
                {
                    Log.Warn("[UtilitySlots][ExtraSlotsVehicles][CyclopsUI] Could not find vanilla Module1/2/3.");
                    return;
                }
                ...
                {
                    Log.Warn("[UtilitySlots][ExtraSlotsVehicles][CyclopsUI] Missing RectTransform on vanilla module slots.");
                    return;
                }
                ...
                if (createdSlots > 0)
                {
                    Log.Info($"[UtilitySlots][ExtraSlotsVehicles][CyclopsUI] Slots créés/mis à jour={createdSlots}, modules={desired}.");
                }
            }
            catch (Exception e)
            {
